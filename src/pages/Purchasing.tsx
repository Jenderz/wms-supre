...
const Purchasing: React.FC = () => {
  ...
  const handleBackToList = () => {
    if (window.confirm('驴Est谩s seguro de volver? Los cambios no guardados se perder谩n.')) {
      setView('list');
    }
  };
  ...
  const handleExportPDF = (lot: PickingLot) => {
    const doc = new jsPDF();
    const storeName = getStore(lot.warehouseId)?.name || 'Almac閚 Desconocida';
    ...
    // Info
    doc.setFontSize(10);
    doc.text(`Almac閚 Destino: ${storeName}`, 14, 45);
    ...
    doc.text(`Descripci贸n: ${lot.description || '-'}`, 14, 66);
    ...
    autoTable(doc, {
      startY: 75,
      head: [['C贸digo', 'Producto', 'Cat.', 'Cant.', 'Bultos', 'Detalle Bultos', 'Medidas']],
      body: tableData,
  ...
  const handleAddProduct = () => {
    ...
    if (items.some(item => item.productId === selectedProduct.id)) {
      alert('Este producto ya est谩 en la lista.');
      return;
    }
  ...
  const handleSave = (status: 'DRAFT' | 'PENDING') => {
    if (!selectedStoreId) return alert('Seleccione una Almac閚 destino');
  ...
  const handleDelete = (id: string) => {
    if (window.confirm('驴Eliminar este lote de picking?')) {
      deletePickingLot(id);
    }
  };
      ...
      <div className="space-y-6">
        <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
          <div>
            ...
            <p className="text-slate-500 mt-2">Gesti贸n de lotes de picking y 贸rdenes de compra.</p>
          </div>
            ...
            <div className="overflow-x-auto -mx-4 sm:mx-0">
              ...
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>Almac閚 Destino</TableHead>
                    ...
                    <TableRow key={lot.id}>
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-medium text-slate-700">{getStore(lot.warehouseId)?.name || 'Almac閚 Desconocida'}</span>
                        </div>
                      </TableCell>
                      <TableCell>
                        <div className="flex items-center gap-2">
        ...
        <div className="space-y-6">
          <div className="bg-white border border-slate-200 rounded-3xl p-4 sm:p-6 shadow-sm">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-2">Almac閚 Destino</label>
                <select
                ...
                >
                  <option value="">Seleccione Almac閚...</option>
                  {warehouses
                    .filter(s => {
                      if (user?.role === 'COMPRAS' && user.assignedWarehouses && user.assignedWarehouses.length > 0) {
